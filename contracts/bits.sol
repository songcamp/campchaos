...
contract BitwiseUtils {
    ...
    function shiftLeft(bytes32 a, uint256 n) internal returns (bytes32) {
        uint256 shifted = uint256(a) * 2 ** n;
        return bytes32(shifted);
    }
    
    // Get bit value at position
    function getBit(bytes32 a, uint256 n) internal returns (bool) {
        return a & shiftLeft(ONE, n) != 0;
    }
    
    // Set bit value at position
    function setBit(bytes32 a, uint256 n) internal returns (bytes32) {
        return a | shiftLeft(ONE, n);
    }
    
    // Set the bit into state "false"
    function clearBit(bytes32 a, uint256 n) internal returns (bytes32) {
